...
struct EnrouteNoteItemList: View {
                            ...
                            ForEach(itemList.indices, id: \.self) { index in
                                ...
                                .swipeActions(allowsFullSwipe: false) {
                                    Button(role: .destructive) {
                                        if let found = coreDataModel.enrouteRefArray.first(where: {$0.parentId == itemList[index].id}) {
    ...
    private func removeQR(_ index: Int) {
        ...
        if let found = coreDataModel.enrouteRefArray.first(where: {$0.parentId == coreDataModel.enrouteArray[index].id}) {
            coreDataModel.delete(found)
        }
        coreDataModel.save()
        resetData()
