...
struct SlideoverAISearchResult: View {
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                VStack(spacing: 0) {
                    ...
                    HStack(alignment: .center) {
                        HStack {
                            Text("AI Search Results").font(.system(size: 17, weight: .semibold)).foregroundColor(Color.black)
                        }
                        Spacer()
                    }.frame(height: 52)
                        .padding(.horizontal)
                    ...
                    List {
                        ...
                        .sheet(isPresented: $showSheet) {
                            ClipboardAISearchDetailModal(isShowing: $showSheet, index: $currentIndex).interactiveDismissDisabled(true)
                        }
    ...
    private func move(from source: IndexSet, to destination: Int) {
        print("Move");
        coreDataModel.dataAISearchFavorite.move(fromOffsets: source, toOffset: destination)
    }
